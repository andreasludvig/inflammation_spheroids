...
# ggsave(filename = here("notebooks/LCMS/output/act-il6.png"), device = "png", dpi = "retina")
```
...
# ggsave(filename = here("notebooks/LCMS/output/act-il1b.png"), device = "png", dpi = "retina")
```
...
EC50_IL6_filtered <- EC50_IL6_filtered %>% filter(time_treatment != 24)
...
new_data <- data.frame(concentration = seq(min(EC50_IL6_filtered$concentration, na.rm = TRUE), 
                                           max(EC50_IL6_filtered$concentration, na.rm = TRUE), length.out = 1000))
...
# Ensure EC50_IL6 is filtered and contains no NA values for the fitting
ic50_il6_data_filtered <- EC50_IL6 %>% filter(!is.na(Concentration) & Concentration > 0 & !is.na(relative_amount))
